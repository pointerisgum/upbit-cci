...
# Keys
access_key = "xwdEMciw0PeGRfpA8xMaVtnVGmFPFxTR6dkKCnUQ"
secret_key = "UOxwdGYVZflyTCbMwrlrzB0Ey44GGxSLl70xp8A4"
...
def get_candle(target_item, tick_kind, inq_range):
    try:
        ...
        # Tick 조회
        # ----------------------------------------
        # querystring = {"market": target_item, "count": inq_range, "to": "2021-10-20 00:00:00"}
...
def get_macd(target_item, tick_kind, inq_range, loop_cnt):
    try:
        ...
        # MACD 계산
        exp1 = df.ewm(span=10, adjust=False).mean()
        exp2 = df.ewm(span=30, adjust=False).mean()
        macd = exp1 - exp2
        exp3 = macd.ewm(span=5, adjust=False).mean()
...
#   1) MACD 값
# -----------------------------------------------------------------------------
def get_macdLocal(candle_datas, loop_cnt):
    try:
...
def get_indicators(target_item, tick_kind, inq_range, loop_cnt):
    try:
        ...
        if len(candle_data) >= 30:
            ...
            # MACD 정보 조회
            macd_data = get_macdLocal(candle_datas, loop_cnt)
